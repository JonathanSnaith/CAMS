...
Partial Class AM_PART_INSPECTIONS_EDIT_RW
    ...
    Protected Sub Page_Init(sender As Object, e As System.EventArgs) Handles Me.Init
        ...
        Me.HF_PDI_ID.Value = Request.QueryString("PDI_ID")
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        If Me.lblRun.Text = "" Then
            Me.lblRun.Text = "1"
            lblPageName.Text = "Asset Management - Edit Part Inspection"
    ...
    End Sub
    Sub GetData()
        Dim dvSql As DataView = DirectCast(SqlDS_GetInspectionData.Select(DataSourceSelectArguments.Empty), DataView)
        For Each drvSql As DataRowView In dvSql
            RadDP_Inspection_Date.SelectedDate = drvSql("PDI_Inspection_Date").ToString()
            RadCB_Inspector.SelectedValue = drvSql("PDI_Inspector").ToString()
            Me.lbl_Overdue_Days.Text = drvSql("PDI_Inspection_Overdue_Days").ToString()
            RadCB_Status.SelectedValue = drvSql("PDI_Status").ToString()
            RadTxt_Comments.Text = drvSql("PDI_Comments").ToString()
            Me.lbl_Model.Text = drvSql("Model").ToString()
            Me.lbl_Part_ID.Text = drvSql("Part").ToString()
            Me.lbl_Component.Text = drvSql("Component").ToString()
        Next
    End Sub
